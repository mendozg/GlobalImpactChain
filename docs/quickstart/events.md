...
looking for. For example, a `MsgEthereumTx` transaction triggers an `event` of type `impactchain` and
...
Impactchain also supports the Ethereum [JSON-RPC](https://eth.wiki/json-rpc/API) filters calls to
...
```bash
impactchaincli rest-server --laddr "tcp://localhost:8545" --node "tcp://localhost:8080" --unlock-key <my_key> --chain-id <chain_id>
```
...
Since Impactchain runs uses Tendermint Core as it's consensus Engine and it's built with the Cosmos
...
PubSubAPI](https://geth.ethereum.org/docs/rpc/pubsub), Impactchain needs to cast the Tendermint
...
```bash
impactchaincli rest-server --laddr "tcp://localhost:8545" --wsport 8546 --unlock-key <my_key> --chain-id <chain_id>
```
...
Learn about Impactchain [accounts](./../basic/accounts.md) {hide}
