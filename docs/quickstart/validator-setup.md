...
```bash
impactchaind tendermint show-validator
```

To create your validator, just use the following command:

```bash
impactchaincli tx staking create-validator \
  --amount=1000000aphoton \
  --pubkey=$(impactchaind tendermint show-validator) \
  --moniker=<impactchain_validator> \
  --chain-id=<chain_id> \
...
3. Link the operator account with a Tendermint node pubkey that will be used for signing blocks. If no `--pubkey` flag is provided, it defaults to the local node pubkey created via the `impactchaind init` command above.
...
Next, craft your `impactchaind gentx` command:
...
::: tip
For more on `gentx`, use the help flag: `impactchaind gentx -h`
:::
...
```bash
impactchaincli query tendermint-validator-set | grep "$(impactchaind tendermint show-validator)"
```

You should now see your validator in one of the block explorers. You are looking for the `bech32`
encoded `address` in the `~/.impactchaind/config/priv_validator.json` file.
...
You can achieve this by setting one of the following flags during when using the `impactchaind start` command:
