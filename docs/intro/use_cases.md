...
Check out the 2 use cases for the Impactchain project. {synopsis}

## Impactchain chain

The Impactchain blockchain provides Ethereum developers to deploy their smart contracts to the
Impactchain EVM and get the benefits of a fast-finality Proof-of-Stake (PoS) chain. Developers will
also benefit from highly-reliable clients from testnets can be used to test and deploy their
contracts.

Impactchain will also offer built-in interoperability functionalities with other Cosmos and BFT chains by using [IBC](https://cosmos.network/ibc). Developers can also benefit from using a bridge network (like
[Chainbridge](https://github.com/ChainSafe/ChainBridge), or a [Peg
Zone](https://github.com/cosmos/peggy)) to enable interoperability between mainnet Ethereum and Impactchain.

## EVM module dependency

The EVM module ([x/evm](https://github.com/mendozg/impactchain/tree/development/x/evm)) packaged inside
Impactchain can be used separately as its own standalone module. This can be added as a dependency to
...
available in the Impactchain chain such as developing a module or importing a third-party one.

Using Impactchain chain will allow for the direct deployment of smart contracts to the Impactchain
network. Utilizing the Impactchain client will defer the chain maintenance to the Impactchain network
and allow for the participation in a more mature blockchain. The Impactchain client will also offer
(in the near future) IBC compatibility which allows for interoperability between different network.

## Next {hide}

Read the available Impactchain [resources](./resources.md) {hide}
