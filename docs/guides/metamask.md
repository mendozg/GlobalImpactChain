...
Connect your Metamask wallet with Impactchain on a localnet mode. {synopsis}

## Start node and REST server

Start the Impactchain node using your terminal:

```bash
impactchaind start --pruning=nothing --rpc.unsafe --log_level "main:info,state:info,mempool:info"
```

::: tip
You can also start a node from scratch by running `./init.sh` from the Impactchain repository directory. This will generate a key called `mykey` that you can use on the next step.
:::
...
```bash
impactchaincli rest-server --laddr "tcp://localhost:8545" --unlock-key mykey --chain-id 1
```

## Adding a custom Network for Impactchain
...
```bash
impactchaincli keys unsafe-export-eth-key mykey
```
...
Network` (or any other than `Localhost 8545` or `Impactchain`) and then switch back to `Impactchain`.
:::
