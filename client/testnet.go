...
func TestnetCmd(ctx *server.Context, cdc *codec.Codec,
	...
	cmd := &cobra.Command{
		Use:   "testnet",
		Short: "Initialize files for a Impactchain testnet",
		Long: `testnet will create "v" number of directories and populate each with
necessary files (private validator, genesis, config, etc.).

Note, strict routability for addresses is turned off in the config file.`,

		Example: "impactchaind testnet --v 4 --keyring-backend test --output-dir ./output --starting-ip-address 192.168.10.2",
	...
	cmd.Flags().String(flagNodeDaemonHome, "impactchaind", "Home directory of the node's daemon configuration")
	cmd.Flags().String(flagNodeCLIHome, "impactchaincli", "Home directory of the node's cli configuration")
	...
	cmd.Flags().String(flagCoinDenom, impactchain.AttoPhoton, "Coin denomination used for staking, governance, mint, crisis and evm parameters")
	cmd.Flags().String(server.FlagMinGasPrices, fmt.Sprintf("0.000006%s", impactchain.AttoPhoton), "Minimum gas prices to accept for transactions; All fees in a tx must meet this minimum (e.g. 0.01aphoton,0.001stake)")
...
func InitTestnet(
	...
	if chainID == "" {
		chainID = fmt.Sprintf("impactchain-%d", tmrand.Int63n(9999999999999)+1)
	}
	...
	for i := 0; i < numValidators; i++ {
		...
		genAccounts = append(genAccounts, impactchain.EthAccount{
